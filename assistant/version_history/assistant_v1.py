...
@Time: 2026-04-27
@Author: 怀风・Halcyon
@Description: 0428，只能做到单步推理（Single-step Reasoning），一个问题中包含两个公司，则只回答了一个；
"""
...
from prompt_template import cus_prompt, system_prompt_agent
...
class StockReportAssistant:
    ...
    def __init__(self):
        self.store = {}
        self.tmp_session_id = "now_user_0428"
    ...
    def run(self):
        ...
        try:
            response = self.agent.invoke(
                {"input": "华为公司和小米公司的股价是多少"},  # 华为的股价是多少
                config={"configurable": {"session_id": self.tmp_session_id}, "max_iterations": 5}
            )
            print(f"\n[个人助手] 分析结果: {response}")
