...
if not API_KEY:
    raise ValueError("未检测到 API_KEY，请检查环境变量是否配置正确")
...
def translation_simplify(state: WorkflowState):
    prompt = f"把下面的建议翻译成英文：{state['simplified_advice']}"
    result = llm.invoke(prompt)
    return {"en_advice": result.content}
...
workflow.add_node("translation", translation_simplify)

workflow.add_edge(START, "generate")
workflow.add_edge("generate", "simplify")
workflow.add_edge("simplify", "translation")
